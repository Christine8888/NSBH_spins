...
if args.free:
    N = 4000
    fixed = {"mu": 1.5, "sigma":100, "m_TOV":[mtov_True,1.7,3.2], "bh_min": bh_min, "bh_slope": bh_slope}
else:
    N = 2000
    fixed = {"mu": 1.5, "sigma":100, "m_TOV":[mtov_True,1.7,3.2], "bh_min": bh_min, "bh_slope": bh_slope, "max_jjkep": max_jjkep, "spin_slope": spin_slope}

for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, 2000, save_to=None, fixed=fixed, mult=True)
    np.savetxt('../{}/{}_mTOV_{}_run_{}_uniform.txt'.format(folder, detector, mtov_True, str(event_counts[i])), samples)
    np.savetxt('../{}/{}_mTOV_{}_run_{}_uniform_likes.txt'.format(folder, detector, mtov_True, str(event_counts[i])), likes)
    if i != 4:
...
print('../{}/{}_mTOV_{}_run_{}_uniform.txt'.format(folder, detector, mtov_True, str(event_counts[i])))
