...
fixed = {"a":0.63, "mu_1": 1.35, "sigma_1":0.07, "mu_2": 1.85, "sigma_2":0.35, "m_TOV":[2,1.5,3], "bh_min": 5, "bh_slope": 2, "max_jjkep":1, "spin_slope":0}

for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, 5000, mult=True, save_to = None, fixed=fixed)
    np.savetxt('./mTOV_2_run_{}_2component.txt'.format(str(event_counts[i])), samples)
    np.savetxt('./mTOV_2_run_{}_2component_likes.txt'.format(str(event_counts[i])), likes)
