...
if args.free:
    N = 5000
    fixed = {"m_TOV":[mtov_True,1.8,3.5], "bh_min":[bh_min, 1.5, bh_min+2], "bh_slope": bh_slope, "max_jjkep": max_jjkep, "spin_slope": spin_slope}
else:
    N = 3000
    fixed = {"mu": 1.5, "sigma": 0.5, "m_TOV":[mtov_True,1.8,3.5], "bh_min":[bh_min, 1.5, bh_min+2], "bh_slope": bh_slope, "max_jjkep": max_jjkep, "spin_slope": spin_slope}

for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, N, mult=True, save_to = None,fixed=fixed)
    np.savetxt('../{}/{}_mTOV_{}_bhmin_{}_{}_1c.txt'.format(folder, detector, mtov_True, bh_min, str(event_counts[i])), samples)
    np.savetxt('../{}/{}_mTOV_{}_bhmin_{}_{}_1c_likes.txt'.format(folder, detector, mtov_True, bh_min, str(event_counts[i])), likes)
    if i != 4:
