...
gw190426 =  np.genfromtxt('./real_data/gw190426_{}_samples.txt'.format(args.direct))
gw190426 = gw190426[np.random.choice(np.arange(gw190426.shape[0]), args.nsamp)]
gw190426 = np.expand_dims(gw190426, 0)

gw200115 =  np.genfromtxt('./real_data/gw200115_{}_samples.txt'.format(args.direct))
gw200115 = gw200115[np.random.choice(np.arange(gw200115.shape[0]), args.nsamp)]
gw200115 = np.expand_dims(gw200115, 0)

gw200105 =  np.genfromtxt('./real_data/gw200105_{}_samples.txt'.format(args.direct))
gw200105 = gw200115[np.random.choice(np.arange(gw200115.shape[0]), args.nsamp)]
gw200105 = np.expand_dims(gw200115, 0)

gw190814 =  np.genfromtxt('./real_data/gw190814_{}_samples.txt'.format(args.direct))
gw190814 = gw200115[np.random.choice(np.arange(gw200115.shape[0]), args.nsamp)]
gw190814 = np.expand_dims(gw200115, 0)
...
fixed = {"mu": 1.5, "sigma":100, "m_TOV":[mtov_True,1.8,4], "bh_min":[bh_min, 1, 10], "bh_slope": [bh_slope, 0, 10], "max_jjkep": [max_jjkep, 0, 1], "spin_slope": [0, -1, 5]}
