...
fixed = {"mu": 1.5, "sigma":100, "m_TOV":[mtov_True,1.8,3.5], "bh_min":[bh_min, bh_min-2, bh_min+2], "bh_slope": bh_slope, "max_jjkep": max_jjkep, "spin_slope": spin_slope}

for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, 4000, mult=True, save_to = None,fixed=fixed)
    np.savetxt('../{}/{}_mTOV_{}_bhmin_{}_{}_u.txt'.format(folder, detector, mtov_True, bh_min, str(event_counts[i])), samples)
    np.savetxt('../{}/{}_mTOV_{}_bhmin_{}_{}_u_likes.txt'.format(folder, detector, mtov_True, bh_min, str(event_counts[i])), likes)
    if i != 4:
