...
def m_crit(m_TOV, j_jkep, ignore_spin=False):
    if ignore_spin:
        return np.repeat(m_TOV, j_jkep.shape[0])
    a2 = 1.316e-1
    a4 = 7.111e-2
...
def generate_NSBH(N, params, nsbh_only = True, vary_slope = False, spinning=False, a_bh = 0.5, pop_type = 'nsbh', spin_params = [1.0, 0.0, 0.0]):
    total = 0
    pop = np.zeros((N, 4))
    ...
    while total < N:
        if spinning:
            s2 = float(1-generate_q(1, spin_params[1], 1-spin_params[2], 1-spin_params[0]))
            s1 = float(1-generate_q(1, spin_params[1], 1-spin_params[2], 1-spin_params[0]))
        else:
...
class Population():
    def __init__(self, params, pop_type, vary_slope=False, selection=False, spinning = False, m1_nospin = False, restrict_m1 = None, spin_params = [1.0, 0.0], ignore_spin = False, verbose=True):
        ...
        if verbose:
            ...
            print('ignore spin: {}'.format(self.ignore_spin))

        if pop_type == "nsbh_one":
    ...
    def pl_spin(self, spin, max_jjkep, spin_slope, min_jjkep = 0):
        ...
        result[mask] = like_m2(1-spin[mask], 1-min_jjkep, 1-max_jjkep, spin_slope)
        result[~mask] = 0
        return result
    ...
    def selection_norm(self, params):
        ...
        if self.spinning:
            spin_likes = self.pl_spin(self.new_set[:,3], self.max_jjkep, self.spin_slope, self.min_jjkep)
            if not self.m1_nospin:
                spin_likes *= self.pl_spin(self.new_set[:,2], self.max_jjkep, self.spin_slope, self.min_jjkep)
    ...
    def set_injection_spins(self, injection_set):
        ...
        if self.spinning:
            if not self.m1_nospin:
                self.new_set[:,2] = 1-generate_q(N, self.spin_slope, 1-self.min_jjkep, 1-self.max_jjkep)
            self.new_set[:,3] = 1-generate_q(N, self.spin_slope, 1-self.min_jjkep, 1-self.max_jjkep)
        else:
            if not self.m1_nospin:
    ...
    def get_samples(self, n, return_p0=False):
        if self.m1_nospin:
            ...
            if self.spinning:
                test_chi_1 = float(1-generate_q(1, self.spin_slope, 1-self.min_jjkep, 1-self.max_jjkep))
            else:
                test_chi_1 = np.random.rand() * self.max_jjkep

        if self.spinning:
            # print('here')
            test_chi_2 = float(1-generate_q(1, self.spin_slope, 1-self.min_jjkep, 1-self.max_jjkep))
        else:
        ...
        if self.pop_type == "nsbh":
            ...
            if self.spinning:
                pop = generate_NSBH(1, params, nsbh_only = True, vary_slope = self.vary_slope, spinning = self.spinning, spin_params = [self.max_jjkep, self.spin_slope, self.min_jjkep])
            else:
            ...
            if self.spinning:
                pop = generate_NSBH(1, params, nsbh_only = True, vary_slope = self.vary_slope, pop_type = 'nsbh_one', spinning = self.spinning, spin_params = [self.max_jjkep, self.spin_slope, self.min_jjkep])
            else:
                ...
                if params[1] > m2range[0] and params[1] < m2range[1]:
                    ...
                    if params[0] > params[1]:
                        ...
                        if self.m1_nospin and self.restrict_m1:
                            ...
                            if params[2] >= 0 and params[2] <= 1.2:
                                if params[3] >= 0 and params[3] <= 1.2:
                                    loglike = loglike_one(params, m_chirp, m_chirp_sigma, mass_ratio, mass_ratio_sigma, chi_eff, chi_eff_sigma)
                                    return loglike
            return -np.inf
        ...
        m_chirp = chirp_mass(samples[:,0], samples[:,1])
        eta = mass_ratio(samples[:,0], samples[:,1])
        chi_eff = chieff(samples[:,0], samples[:,1], samples[:,2], samples[:,3])
        q = samples[:,1]/samples[:,0]

        samples = np.vstack([m_chirp, eta, chi_eff, q]).T
        print(" posteriors are flat in {m1, m2, a1, a2}")
    ...
    def event_likelihood_nsbh_one_samples(self, samples, params, nomean=False):
        ...
        if self.vary_slope:
            ...
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit_slope(params[2], params[5], samples[:,3]), minBH = params[3])
            if self.spinning:
                maxspin = params[6]
            ...
            p_m2 = truncnormal_like(samples[:,1], params[0], params[1], lower = 1, upper = m_crit(params[2], samples[:,3], self.ignore_spin))
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit(params[2], samples[:,3], self.ignore_spin), minBH = params[3])
            if self.spinning:
                maxspin = params[5]
    ...
    def event_likelihood_nsbh_samples(self, samples, params, nomean=False):
        ...
        if self.vary_slope:
            ...
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit_slope(params[5], params[8], samples[:,3]), minBH = params[6])
            if self.spinning:
                maxspin = params[9]
                # spin_likes = self.uniform_spin(samples[:,2], params[9]) * self.uniform_spin(samples[:,3], params[9])
            ...
            p_m2 = two_truncnormal_like(samples[:,1], a = params[0], mu_1 = params[1], sigma_1 = params[2], \
                                      mu_2 = params[3], sigma_2 = params[4], lower = 1, upper = m_crit(params[5], samples[:,3], self.ignore_spin))
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit(params[5], samples[:,3], self.ignore_spin), minBH = params[6])
            if self.spinning:
                maxspin = params[8]
                # spin_likes = self.uniform_spin(samples[:,2], params[8]) * self.uniform_spin(samples[:,3], params[8])
    ...
    def event_likelihood_nsbh_vec(self, samples, params, mu):
        ...
        if self.vary_slope:
            p_q = like_beta_nsbh(q, self.beta, maxNS =  m_crit_slope(params[5], params[8], i[:,3]), minBH = params[6])
            ...
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit(params[5], i[:,3], self.ignore_spin), minBH = params[6])
            if self.spinning:
                maxspin = params[8]
    ...
    def event_likelihood_nsbh_one_vec(self, samples, params, mu):
        ...
        if self.vary_slope:
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit_slope(params[2], params[5], i[:,3]), minBH = params[3])
            ...
            p_m2 = truncnormal_like(i[:,1], params[0], params[1], lower = 1, upper = m_crit(params[2], i[:,3], self.ignore_spin))
            p_q = like_beta_nsbh(q, self.beta, maxNS = m_crit(params[2], i[:,3], self.ignore_spin), minBH = params[3])
