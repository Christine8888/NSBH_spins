...
fixed = {"mu":1.4, "sigma":0.5, "m_TOV":[2,1.5,3], "bh_min": 5, "bh_slope": 2, "max_jjkep":1, "spin_slope":0}
for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, 5000, save_to=None, fixed=fixed, mult=True)
    np.savetxt('./mTOV_2_run_{}.txt'.format(str(event_counts[i])), samples)
    np.savetxt('./mTOV_2_run_{}_likes.txt'.format(str(event_counts[i])), likes)

    new_samples = nsbh_population.get_population(10, True)
    pop_samples = np.vstack([pop_samples, new_samples])
    print(pop_samples.shape)

pop_samples = pop_samples.reshape((50,3000,4))
np.savetxt('./mTOV_2_run_{}_popsamples'.format(str(event_counts[i])), pop_samples)
