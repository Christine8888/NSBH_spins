...
nsbh_population = p.Population([1.5, 0.5, mtov_True, 1, 3, bh_min, bh_slope, slope], 'nsbh_one', vary_slope=True, selection=True, m1_nospin = True, spinning=True, spin_params=[max_jjkep, spin_slope])
nsbh_population.set_injection_spins(p.injection_set)

pop_samples = nsbh_population.get_population(event_counts[0], True)

if args.free:
    fixed = { "m_TOV":[mtov_True,1.7,3.2], "slope":[slope, 0.0, 1.0], "bh_min": bh_min, "bh_slope": bh_slope}
else:
    fixed = {"mu": 1.5, "sigma":0.5, "m_TOV":[mtov_True,1.7,3.2], "slope":[slope, 0.0, 1.0], "bh_min": bh_min, "bh_slope": bh_slope, "max_jjkep": max_jjkep, "spin_slope": spin_slope}

for i in range(5):
    samples, likes = nsbh_population.infer(pop_samples, 4000, mult=True, save_to = None,fixed=fixed)
    np.savetxt('../{}/{}_mTOV_{}_run_{}_1c_slope_{}.txt'.format(folder, detector, mtov_True, str(event_counts[i]), slope), samples)
    np.savetxt('../{}/{}_mTOV_{}_run_{}_1c_slope_{}_likes.txt'.format(folder, detector, mtov_True, str(event_counts[i]), slope), likes)
    if i != 4:
...
print('../{}/{}_mTOV_{}_run_{}_u_slope_{}.txt'.format(folder, detector, mtov_True, str(event_counts[i]),slope))
